...
| [RFC-0013](./RFC-0013-convencao-naming-de-props-booleanas.md) | Convenção de naming de props booleanas | R5 — gate R6 | 2026-04-24 | 2026-04-24 | API canônica sem `is*` em `Field`/`Dialog`/contexto Accordion. Aliases legados (`isDisabled`/`isRequired`/`isInvalid`/`isOpen`) aceitos com `console.warn` em dev. Todos consumidores internos migrados. **Complementada por RFC-0030 (2026-05-02)** — varredura final em overlays remanescentes, contextos `is*` e `Pagination.isActive`. |
| [RFC-0032](./RFC-0032-feedback-tones-cross-componente.md) | Catálogo `FeedbackTone` cross-componente | R7 #7 + R8 (sub-onda 8.D) | 2026-05-02 | 2026-05-02 | Tipo canônico em foundations + helper `getFeedbackToneColor` substituindo 5 mapeadores locais (Alert/Toast/Badge/ProgressBar/ProgressCircle/Tag/Chip). Subset documentado por componente. Lint guard `pnpm test:feedback-tones`. Habilita TD-030 + TD-034. 962/962 verdes (+22 helper). |
| [RFC-0033](./RFC-0033-chip-interativo.md) | Chip-Interativo: contrato explícito para Chip selecionável | R8 review (CH-Bug-1) | 2026-05-02 | 2026-05-02 | Discriminated union `ChipDecorativeProps \| ChipSelectableProps`. Default decorativo (`<span>`); `selectable={true}` ativa `<button>` focável com `aria-pressed` + Space/Enter (web) e `accessibilityState.selected` (native). `Chip.Remove` ramifica anatomia automaticamente para evitar nested-button. Web `chip.tsx` + novo `chip.native.tsx` paritário. 974/974 verdes (+12 chip). Habilita TD-034. |
| [RFC-0030](./RFC-0030-completar-rfc-0013-naming-overlays.md) | Completar RFC-0013 em overlays e contextos remanescentes | revisão `arbor-ds-arch` | 2026-05-02 | 2026-05-02 | `Drawer`/`Tooltip`/`Popover`/`Menu` migrados para `open`/`onOpenChange` (sem `isOpen`/`onClose`); contextos `Select`/`Checkbox`/`Radio`/`ButtonGroup` migrados para naming sem `is*`; `PaginationItemProps.isActive` → `current` (alinhado a `aria-current="page"`); `useDisclosure().isOpen` documentado como exceção. |
