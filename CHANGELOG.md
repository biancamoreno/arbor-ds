...
- **RFC-0041 PR1 — Polish visual default (foundations + Button piloto):** refinamento coordenado em 8 eixos. Pré-v1 sem consumidores externos; sem aliases legacy. Override pelo tema continua trivial (`createTheme(themeLight, { ... })`).
  - **Brand default trocado de `aqua` para `ultraviolet`.** `themeLight.colors.brand.solid` agora é `#3E3196` (ultraviolet 90, derivado de Ultraviolet 70 `#6352E1` apresentado como "Linear-feel" na RFC §3.8). `themeDark` segue simétrico via `makeDarkColorScale`. Primitive `ultraviolet` estendida com steps `110` e `120` (derivados linearmente de `100` ↔ preto). Para reverter: `createTheme(themeLight, { colors: { brand: createBrandPalette('#1BA285').light } })`.
  ...
  - **Button piloto:** `tokens.components.button.fontWeight` 'medium' → 'semibold'; ganhou bloco `hover: { translateY: '-1px', shadow: 'sm' }`. Recipe `button` em `base-theme.ts` ganhou `_hover` (translateY + boxShadow alias) e `_active` (reset). Lista de propriedades em transição inclui `box-shadow`. Aplicável só a `<Button>` no PR1; sweep para os demais ~25 componentes vai no PR2.
  - **Storybook:** nova página `Foundations/Polish v1 — Brand candidates` com comparativo lado-a-lado de Sapphire 70 / Ultraviolet 70 (default) / Violet 70. Ultraviolet é a recomendação do arquiteto e o default já aplicado em `themeLight`; aprovação destrava PR2.
